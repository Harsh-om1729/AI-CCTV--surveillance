...
Phase-by-phase build notes: what was tried, what was measured, what broke and
why, and what shipped instead. This is the detailed lab notebook behind
IBVAP — for a project overview, quick start, and contributor guide, see the
[root README](../README.md).

Every claim below is either unit-tested or **live-verified** against a real
camera feed, and says which.
...
- [x] Phase 22 - Deployability (`app.py --headless`/`IBVAP_HEADLESS=1` skips
      the OpenCV window entirely and turns SIGTERM into the same clean-
      shutdown path Ctrl+C already used, for `systemctl stop`; the app.py/
      dashboard camera collision was already resolved by Phase 20's runtime
      publisher, so only the unused Streamlit fallback still has it;
      `integration/api.py` now serves the built `frontend/dist/` itself when
      present, so the API and dashboard are one systemd service instead of
      two. Sample units and full install walkthrough: `deploy/` and
      `docs/DEPLOY.md`. Not yet live-tested on real systemd - this Mac is
      Darwin - so treat the unit files as reviewed-but-unverified until run
